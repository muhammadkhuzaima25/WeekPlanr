...
import { Sparkles, Bot, Frown, CheckCircle2 } from 'lucide-react'
...
function TaskMotionCard({ task, onToggle, isDraggable, dragProvided, dragSnap }) {
  ...
  const content = (
    <motion.div
      ...
      style={{
        ...
        ...(dragProvided ? dragProvided.draggableProps.style : {}),
      }}
    >
...
export default function Dashboard() {
  ...
  const handleDragEnd = async (result) => {
    ...
    const prev = { ...weekTasks }
    const newTasks = { ...weekTasks }
    const srcArr = [...(newTasks[source.droppableId] || [])]
    const [moved] = srcArr.splice(source.index, 1)
    newTasks[source.droppableId] = srcArr
    const destArr = [...(newTasks[destination.droppableId] || [])]
    destArr.splice(destination.index, 0, moved)
    newTasks[destination.droppableId] = destArr
    setWeekTasks(newTasks)
    try {
      await api.put(`/tasks/${draggableId}`, { scheduledDay: destination.droppableId })
    } catch {
      setWeekTasks(prev)
  ...
  const handleOverwhelmed = () => setShowOverwhelm(true)
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>
          ...
          <div style={{
            ...
            <span>Quick Tip: Tap the circle tick on any task card to cross it out and smoothly shift it to the Completed section below. Any task that crosses its deadline without being marked will be automatically archived by the system!</span>
          </div>
        </header>
                  ...
                  <Droppable key={day} droppableId={day}>
                    {(provided, snapshot) => (
                      <div ref={provided.innerRef} {...provided.droppableProps} style={{
                        background: snapshot.isDraggingOver ? 'rgba(0,201,138,0.08)' : isToday ? 'linear-gradient(180deg, rgba(0,201,138,0.06), var(--bg-card))' : 'var(--bg-card)',
                        ...
                        {isToday && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(90deg, #00C98A, #00A370)' }} />}
                        <div style={{ textAlign: 'center', marginBottom: 6 }}>
                          <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', color: isToday ? '#00C98A' : 'var(--text-muted)' }}>{day}</div>
                          ...
                          <AnimatePresence mode="popLayout">
                            {dayTasks.map((task, j) => (
                              <Draggable key={task._id} draggableId={task._id} index={j}>
                                {(prov, snap) => (
                                  <TaskMotionCard task={task} onToggle={handleToggleTask} isDraggable dragProvided={prov} dragSnap={snap} />
                                )}
                              </Draggable>
                            ))}
                          </AnimatePresence>
